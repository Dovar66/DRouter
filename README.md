...
### 如何使用组件化逐步改造你的项目？
    ...
    完全的组件化拆分非一两日就能完成，而我们的项目却总会不断有新的需求等待开发，版本迭代工作几乎注定了我们不可能将项目需求暂停来做组件化。
    所以需要我们协调好版本迭代与组件化拆分的同步进行，下面是根据我的组件化经历得出的建议：
    
    准备：
    1.新增APP壳工程，建议参考本项目中的app工程.
    2.将你项目当前的application工程作为公共服务层(后面直接用common_service表示)，当然，由于还未开始拆分组件，所以此时它也是最大的业务组件.
    3 新增一个组件(后面用module_search表示)，建议优先选择一个自己最熟悉或相对简单的业务模块着手，比如我自己公司项目的搜索模块，它只有搜索功能且与其他模块交互很少，所以我选择由它开始。
